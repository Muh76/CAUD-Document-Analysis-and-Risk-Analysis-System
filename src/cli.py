...
def prepare(
    config_path: str = typer.Option(
        "configs/parse.yaml", "--config", "-c", help="Path to config file"
    ),
    output_dir: str = typer.Option(
        "data/processed", "--output", "-o", help="Output directory"
    ),
    validate: bool = typer.Option(
        True, "--validate/--no-validate", help="Run data validation"
    ),
):
    ...
    try:
        ...
        typer.echo(
            f"📊 Processed {len(all_metadata)} contracts, {len(all_clauses)} clauses"
        )
...
def extract(
    ...
    input_dir: str = typer.Option(
        "data/processed", "--input", "-i", help="Input directory"
    ),
    output_dir: str = typer.Option(
        "data/enriched", "--output", "-o", help="Output directory"
    ),
):
...
def validate(
    ...
    data_dir: str = typer.Option(
        "data/processed", "--data", "-d", help="Data directory to validate"
    ),
...
def train(
    config_path: str = typer.Option(
        "configs/models.yaml", "--config", "-c", help="Path to config file"
    ),
    data_dir: str = typer.Option(
        "data/processed", "--data", "-d", help="Data directory"
    ),
    model_dir: str = typer.Option(
        "models/baseline", "--model", "-m", help="Model output directory"
    ),
    model_type: str = typer.Option(
        "logistic", "--type", "-t", help="Model type (logistic, random_forest)"
    ),
):
...
def analyze(
    ...
    model_dir: str = typer.Option(
        "models/baseline", "--model", "-m", help="Model directory"
    ),
    output_dir: str = typer.Option(
        "analysis_results", "--output", "-o", help="Output directory"
    ),
):
    ...
    try:
        # Load models
        model_path = Path(model_dir)
        classifier = BaselineClauseClassifier.load_model(
            model_path / "baseline_classifier.joblib"
        )
        risk_scorer = KeywordRiskScorer()
        ...
        typer.echo(
            f"  High risk clauses: {sum(1 for r in results if r['risk_level'] == 'HIGH')}"
        )
        typer.echo(
            f"  Medium risk clauses: {sum(1 for r in results if r['risk_level'] == 'MEDIUM')}"
        )
        typer.echo(
            f"  Low risk clauses: {sum(1 for r in results if r['risk_level'] == 'LOW')}"
        )
