...
from src.config.config import OPENAI_API_KEY, AZURE_OPENAI_ENDPOINT, AZURE_OPENAI_API_KEY
...
class EnhancedLegalRAGSystem:
    ...
    def find_similar_clauses(self, query_clause: str, clause_type: str, top_k: int = 5) -> List[Dict]:
    ...
    def suggest_alternative_wording(self, risky_clause: str, clause_type: str, use_azure: bool = False) -> List[str]:
        """Suggest alternative wording using OpenAI or Azure"""
        try:
            # Find similar clauses first
            similar_clauses = self.find_similar_clauses(risky_clause, clause_type, top_k=3)
            ...
            response = client.chat.completions.create(
                model=model,
                messages=[
                    {"role": "system", "content": "You are a legal expert specializing in contract review and risk mitigation."},
    ...
    def analyze_clause_risk(self, clause_text: str, clause_type: str, use_azure: bool = False) -> Dict:
        ...
        try:
            ...
            response = client.chat.completions.create(
                model=model,
                messages=[
                    {"role": "system", "content": "You are a legal risk analyst. Respond with valid JSON only."},
                    {"role": "user", "content": prompt}
                ],
                max_tokens=300,
                temperature=0.1
