...
class BaselineClauseClassifier:
    ...
    def train_model(
        ...
        logger.info(
            f"CV F1 Macro: {cv_scores.mean():.3f} (+/- {cv_scores.std() * 2:.3f})"
        )
...
class KeywordRiskScorer:
    ...
    def __init__(self):
        ...
        self.risk_weights = {"high_risk": 3.0, "medium_risk": 2.0, "low_risk": 1.0}
    ...
    def score_contract(self, clauses: List[str]) -> Dict[str, Any]:
            ...
            clause_scores.append(
                {
                    "clause_index": i,
                    "clause_text": (
                        clause[:100] + "..." if len(clause) > 100 else clause
                    ),
        ...
        high_risk_clauses = sum(
            1 for score in clause_scores if score["risk_level"] == "HIGH"
        )
        medium_risk_clauses = sum(
            1 for score in clause_scores if score["risk_level"] == "MEDIUM"
        )
...
class BaselineEvaluator:
    ...
    def evaluate_classifier(
        self,
        classifier: BaselineClauseClassifier,
        X_test: np.ndarray,
        y_test: np.ndarray,
    ) -> Dict[str, Any]:
        ...
        class_report = classification_report(
            y_test, y_pred, target_names=classifier.clause_types, output_dict=True
        )
    ...
    def evaluate_risk_scorer(
        self, risk_scorer: KeywordRiskScorer, test_clauses: List[str]
    ) -> Dict[str, Any]:
        ...
        for score in clause_scores:
            for risk in score["detected_risks"]:
                ...
                risk_type_distribution[risk_type] = (
                    risk_type_distribution.get(risk_type, 0) + 1
                )
        ...
        results = {
            ...
            "clauses_with_risks": sum(
                1 for score in clause_scores if score["detected_risks"]
            ),
            "evaluation_timestamp": datetime.now().isoformat(),
        }
