...
class RiskScorer:
    ...
    def _calculate_clause_risk(self, clause_text: str, clause_type: str) -> float:
        ...
        high_risk_count = sum(1 for term in indicators['high_risk'] if term in clause_lower)
        medium_risk_count = sum(1 for term in indicators['medium_risk'] if term in clause_lower)
        low_risk_count = sum(1 for term in indicators['low_risk'] if term in clause_lower)
    ...
    def prioritize_clauses(self, extracted_clauses: Dict[str, str]) -> List[Dict]:
        ...
        prioritized.sort(key=lambda x: x['business_impact']['total_impact_usd'], reverse=True)
    ...
    def generate_risk_report(self, extracted_clauses: Dict[str, str]) -> Dict:
        ...
        return {
            ...
            'recommendations': self._generate_risk_recommendations(risk_level, prioritized_clauses),
            'summary': {
                'total_clauses': len(extracted_clauses),
                'high_risk_clauses': len([c for c in prioritized_clauses if c['risk_level'] in ['high', 'critical']]),
                'estimated_review_time': business_impact['time_impact_hours'],
                'estimated_cost': business_impact['total_impact_usd']
            }
        }
    
    def _generate_risk_recommendations(self, risk_level: str, prioritized_clauses: List[Dict]) -> List[str]:
        """
        ...
        for clause in prioritized_clauses[:3]:  # Top 3 highest risk clauses
            if clause['risk_level'] in ['high', 'critical']:
                recommendations.append(f"Review {clause['clause_type']} clause for modifications")
