...
async def analyze_contract(
    ...
    try:
        ...
        if not file_processor.is_valid_file(file):
            raise HTTPException(
                status_code=400, 
                detail="Invalid file type. Supported: PDF, TXT, DOCX"
            )
...
async def analyze_contracts_batch(
    ...
    return {
        ...
        "summary": {
            ...
            "average_risk_score": sum([r.get("risk_score", 0) for r in results if "error" not in r]) / len([r for r in results if "error" not in r]) if results else 0
        }
    }
