...
class ContractDataPipeline:
    ...
    def extract_metadata(self, text: str, contract_id: str) -> ContractMetadata:
        ...
        total_clauses = sum(
            len(re.findall(pattern, text, re.IGNORECASE)) for pattern in clause_patterns
        )
    ...
    def _classify_clause_type(self, header: str, text: str) -> Optional[str]:
        ...
        for clause_type, keywords in clause_types.items():
            if any(
                keyword in header_lower or keyword in text_lower for keyword in keywords
            ):
                return clause_type
    ...
    def _identify_risk_flags(self, text: str) -> List[str]:
        ...
        risk_patterns = {
            ...
            "unfair_terms": [
                r"\b(?:unfair|unreasonable|excessive)\s+(?:terms|conditions)"
            ],
        }
    ...
    ) -> Tuple[ContractMetadata, List[ClauseSegment]]:
    ...
    def generate_data_report(
        ...
        clauses_list: List[List[ClauseSegment]],
    ) -> Dict[str, Any]:
            ...
            for clause in clauses:
                for entity in clause.entities:
                    entity_types[entity["label"]] = (
                        entity_types.get(entity["label"], 0) + 1
                    )
        ...
        return {
            "summary": {
                ...
                "total_clauses": total_clauses,
                "avg_clauses_per_contract": (
                    total_clauses / total_contracts if total_contracts > 0 else 0
                ),
                "processing_timestamp": datetime.now().isoformat(),
            },
            ...
            "data_quality": {
                "contracts_with_metadata": sum(
                    1 for m in metadata_list if m.contract_type
                ),
                "clauses_with_type": sum(
                    1
                    for clauses in clauses_list
                    for clause in clauses
                    if clause.clause_type
                ),
                "clauses_with_entities": sum(
                    1
                    for clauses in clauses_list
                    for clause in clauses
                    if clause.entities
...
def main():
    ...
    logger.info(
        f"Processed {len(all_metadata)} contracts with {sum(len(c) for c in all_clauses)} clauses"
    )
