...
try:
    ...
    GE_AVAILABLE = False
    print("⚠️ Great Expectations not available - using Pandera only")
...
class DataValidator:
    ...
    def setup_great_expectations(self):
        ...
        if not GE_AVAILABLE:
            self.logger.warning("Great Expectations not available - skipping GE setup")
    ...
    def create_pandera_schemas(self) -> Dict[str, DataFrameSchema]:
        ...
        # Contract metadata schema - UPDATED TO MATCH ACTUAL DATA
        contract_metadata_schema = DataFrameSchema(
            {
                ...
                "parties": Column(str, nullable=False),  # String representation of list
        ...
        # Clause segments schema - UPDATED TO MATCH ACTUAL DATA
        clause_segments_schema = DataFrameSchema(
            {
                "contract_id": Column(str, nullable=False),
                "clause_type": Column(str, nullable=False),
                "text": Column(
                    str, Check.str_length(min_value=10, max_value=5000), nullable=False
                ),
        ...
        # Enriched metadata schema - FOR FUTURE USE
        ...
        try:
            ...
            if metadata_path.exists():
                ...
                try:
                    ...
                    validation_results["contract_metadata"] = {
                        "passed": True,
                        "error": None,
                    }
            ...
            if clauses_path.exists():
                ...
                try:
                    ...
                    validation_results["clause_segments"] = {
                        "passed": True,
                        "error": None,
                    }
            ...
            if enriched_path.exists():
                ...
                try:
                    ...
                    validation_results["enriched_metadata"] = {
                        "passed": True,
                        "error": None,
                    }
    ...
    def create_great_expectations_suite(
        self, suite_name: str = "contract_analysis_suite"
    ):
    ...
    def validate_with_great_expectations(self, data_dir: str) -> Dict[str, Any]:
        ...
        if not GE_AVAILABLE:
            return {"ge_available": False, "error": "Great Expectations not available"}

        if not self.ge_context:
            return {
                "ge_validation": {
                    "passed": False,
                    "error": "Great Expectations not initialized",
                }
            }
        ...
        try:
            ...
            if not suite:
                return {
                    "ge_validation": {
                        "passed": False,
                        "error": "Could not create validation suite",
                    }
                }
                ...
                "validations": [
                    {
                        "batch_request": {
                            ...
                            "data_asset_name": "contract_metadata",
                            "runtime_parameters": {
                                "path": f"{data_dir}/contract_metadata.csv"
                            },
                            "batch_identifiers": {
...
def main():
    ...
    parser.add_argument(
        "--data_dir", default="data/processed", help="Data directory to validate"
    )
    parser.add_argument(
        "--config", default="configs/parse.yaml", help="Configuration file"
    )
    parser.add_argument(
